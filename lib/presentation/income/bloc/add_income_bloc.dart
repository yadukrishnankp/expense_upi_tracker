...
class AddIncomeBloc extends Bloc<AddIncomeEvent,AddIncomeState>{
  ...
  final PreferenceRepo preferenceRepo;
    AddIncomeBloc ({ required this.preferenceRepo, required this.addExpenseUseCase}) :super (AddIncomeState.initial(addIncomeData: AddIncomeData(
        amount: 0,
      category: "Salary",
      wallet: "Bank",
    ...
    _addIncome(IncomeAddEvent event,Emitter<AddIncomeState> emit)async{
      ...
      final result =await  addExpenseUseCase.call(TransactionModel(id: id, userId: userId?? "",category: state.addIncomeData.category, wallet: state.addIncomeData.wallet, type: "Income", dateTime: state.addIncomeData.dateTime, createdTime: DateTime.now(), amount: state.addIncomeData.amount,
